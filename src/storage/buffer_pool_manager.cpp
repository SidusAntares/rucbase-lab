 ...
 */
void BufferPoolManager::UpdatePage(Page *page, PageId new_page_id,
...
bool BufferPoolManager::FlushPage(PageId page_id) {
  ...
  disk_manager_->write_page(page_id.fd, page_id.page_no, page->GetData(),
                            PAGE_SIZE);
  page->is_dirty_ = false;
...
Page *BufferPoolManager::NewPage(PageId *page_id) {
  ...
  // 3.   Pick a victim page P from either the free list or the replacer. Always
...
void BufferPoolManager::FlushAllPages(int fd) {
  ...
  for (size_t i = 0; i < pool_size_; i++) {
    ...
    if (page->GetPageId().fd == fd &&
      ...
      disk_manager_->write_page(page->GetPageId().fd, page->GetPageId().page_no,
                                page->GetData(), PAGE_SIZE);
      page->is_dirty_ = false;
    }
  }
