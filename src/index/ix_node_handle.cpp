...
int IxNodeHandle::lower_bound(const char *target) const {
  ...
  while (left < right) {
    ...
    if (ix_compare(get_key(mid), target, file_hdr->col_type,
                   file_hdr->col_len) < 0) {
      left = mid + 1;
    } else {
      right = mid;
...
void IxNodeHandle::insert_pairs(int pos, const char *key, const Rid *rid,
  ...
  assert(n >= 0 && page_hdr->num_key + n <= GetMaxSize());

  int move_num = page_hdr->num_key - pos;
  if (move_num > 0) {
    memmove(get_key(pos + n), get_key(pos), move_num * file_hdr->col_len);
...
void IxNodeHandle::erase_pair(int pos) {
  ...
  if (move_num > 0) {
    memmove(get_key(pos), get_key(pos + 1), move_num * file_hdr->col_len);
